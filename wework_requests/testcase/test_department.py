...
class TestDepartment():
    def setup(self):
        ...
        corpsecret_info = yaml.safe_load(open("config.yaml"))
        self.department.get_token(corpsecret_info["token"]["department_secret"])

    @allure.feature("创建部门")
    @pytest.mark.parametrize("department_id,department_name",[(4, "技术部")],ids={"创建部门"})
